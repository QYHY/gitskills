...
vector<pair<int, int> > MultiImages::getFeaturePairsBySequentialRANSAC(const pair<int, int> & _match_pair,
    ...
    findHomography(_X, _Y, RANSAC, parameter.global_homography_max_inliers_dist, final_mask, GLOBAL_MAX_ITERATION);
    ...
    while(tmp_X.size() >= HOMOGRAPHY_MODEL_MIN_POINTS &&
        ...
        findHomography(tmp_X, tmp_Y, RANSAC, parameter.local_homogrpahy_max_inliers_dist, mask, LOCAL_MAX_ITERATION);
